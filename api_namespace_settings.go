...
func autonomyRequestID(value string) string {
	...
	if _, err := rand.Read(id[:]); err != nil {
		// Match uuid.NewString's fail-closed behavior: sending an empty or
		// predictable idempotency key is more dangerous than stopping the call.
		panic(fmt.Errorf("mockarty: generate autonomy request id: %w", err))
	}
	id[6] = (id[6] & 0x0f) | 0x40
	id[8] = (id[8] & 0x3f) | 0x80
