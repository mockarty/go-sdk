...
func TestAutonomyRequestIDUsesStableValueOrUUIDv4(t *testing.T) {
	if got := autonomyRequestID("stable-retry-id"); got != "stable-retry-id" {
		t.Fatalf("explicit request id changed to %q", got)
	}
	generated := autonomyRequestID("")
	if len(generated) != 36 || generated[8] != '-' || generated[13] != '-' ||
		generated[18] != '-' || generated[23] != '-' || generated[14] != '4' ||
		!strings.Contains("89ab", string(generated[19])) {
		t.Fatalf("generated request id is not RFC 4122 UUIDv4: %q", generated)
	}
}
